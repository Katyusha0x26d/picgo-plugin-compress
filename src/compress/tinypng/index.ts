...
export function TinypngKeyCompress(ctx: IPicGo, { imageUrl, key, stripExif = true }: CommonParams & ITinypngOptions): Promise<ImageInfo> {
  return Tinypng.init({ ctx, keys: key!.split(',') })
    .then(() => Tinypng.upload(imageUrl, stripExif))
    .then((buffer) => {
